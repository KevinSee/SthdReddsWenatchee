...
spwn_rch = redd_results %>%
  ...
  mutate(Location = factor(Location, levels = c('Below_TUM',
         ...
         across(Location,
                fct_recode,
                "Below TUM (mainstem)" = "Below_TUM",
                "Above TUM (mainstem)" = "TUM_bb")) %>%
...
```{r covariate-comparison, fig.cap = "Net error covariate values. Colors correspond to either the original study or the reaches where the model was used in this report."}
...
fpr_df %>%
  ...
  mutate(Location = recode(Location, 
                           ...
                           'Below_TUM' = 'Below TUM (mainstem)',
                           'TUM_bb' = 'Above TUM (mainstem)')) %>%
  kable(digits = 3,
        booktabs = T,
...
Fish collected for broodstock at Wells Hatchery and within the Wenatchee had a known sex. Based on PIT tag detections of this broodstock collection, those sexes could be compared with the sex call at Priest when the fish was initially tagged. Assuming that this error rate is systematic across the entire run within a brood year, we combined all tags with this kind of comparison. Table \@ref(tab:sex-err) shows the sample sizes and results. 
